...
class SemioticController {
  ...
  async delete(req, res) {
    ...
    try {
      await Semiotic.deleteById(idSemiotica);
  
      res.json({ mensagem: "Marcação semiotica deletada com sucesso!" });
  
    } catch (err) {
      res.status(500).json({ 
        erro: "Erro ao deletar marcação semiotica", 
